...
本项目基于 [nanoGPT](https://github.com/karpathy/nanoGPT) 框架，使用自有的多角色对话数据集（Axiodrasil 系统日志）进行指令微调（Supervised Fine-Tuning, SFT），使模型学会模仿不同角色的说话风格（如 Bina、Taki、Chizheng 等）。项目包含完整的数据清洗、数据准备、训练脚本及示例数据，可作为个性化对话模型训练的基础。
...
```
.
├── clean_axio.py          # 数据清洗脚本（从原始 JSON 提取对话对）
├── train_sft.py           # 微调训练脚本（支持 mask 损失）
├── data/
│   └── sft/
│       ├── prepare.py     # 数据准备脚本（生成 train.bin, val.bin 及 mask）
│       └── sample.jsonl   # 示例数据（前10条对话，展示格式）
└── README.md              # 本文档
```
...
由于隐私原因，完整数据未包含在仓库中，但提供了样本数据 `data/sft/sample.jsonl`，可参考格式准备自己的数据。
...
## 仓库链接
[https://github.com/TraitYoung/NanoChat-SFT](https://github.com/TraitYoung/NanoChat-SFT)
```
...
3. **获取仓库链接**  
   你的仓库 HTTPS 链接为：  
   `https://github.com/TraitYoung/NanoChat-SFT`  
   可将此链接直接放入简历的“项目经历”部分。
